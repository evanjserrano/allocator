...
void freem(void* ptr);
# 2 "alloc.c" 2
# 1 "/Library/Developer/CommandLineTools/usr/lib/clang/11.0.3/include/stdbool.h" 1 3
# 3 "alloc.c" 2
...
void print_heap();
_Bool is_allocated(preamble_t);

void* heap_start_g = ((void *)0);
static const size_t BLOCK_SIZE = 0x20UL;
static const size_t MAX_ALLOC = 0x8UL;

_Static_assert(0x20UL % 0x8UL == 0,
               "BLOCK_SIZE must be divisible by MAX_ALLOC");
# 40 "alloc.c"
inline _Bool is_allocated(preamble_t preamble)
{
...
void* get_chunk(size_t size)
{
    ...
    if (heap_start_g == ((void *)0))
    {
        fprintf(__stderrp, "%s(): \t", __FUNCTION__); fprintf(__stderrp, "Initializing Heap\n");
    ...
    if (size == 0)
    {
        fprintf(__stderrp, "%s(): \t", __FUNCTION__); fprintf(__stderrp, "Size is 0\n");
        return ((void *)0);
    }
    if (size > MAX_ALLOC)
    {
        fprintf(__stderrp, "%s(): \t", __FUNCTION__); fprintf(__stderrp, "Size (%zu) too large (size > %zu)\n", size, MAX_ALLOC);
        return ((void *)0);
    }


    fprintf(__stderrp, "%s(): \t", __FUNCTION__); fprintf(__stderrp, "Searching for free chunk of memory\n");
    ...
    while (curr_chunk < prog_break)
    {
        ...
        if (chunk_size >= size)
        {
            fprintf(__stderrp, "%s(): \t", __FUNCTION__); fprintf(__stderrp, "Free chunk found: %p\n", curr_chunk);
            return curr_chunk;
        }
    ...
    fprintf(__stderrp, "%s(): \t", __FUNCTION__); fprintf(__stderrp, "No free chunk found... allocating more memory\n");
    sbrk(BLOCK_SIZE);
...
void* allocm(size_t size)
{
    fprintf(__stderrp, "%s(): \t", __FUNCTION__); fprintf(__stderrp, "size = %zu\n", size);


    size_t chunk_size = size + sizeof(preamble_t);
    void* chunk = get_chunk(chunk_size);
    preamble_t rem;
    if (chunk == ((void *)0) || (rem = *(preamble_t*)chunk - chunk_size) < 0)
    {
        return ((void *)0);
    }
...
void print_heap()
{
    fprintf(__stderrp, "%s(): \t", __FUNCTION__); fprintf(__stderrp, "\n");
